const App = {
  ...
  // ── Featured Listings (index.html) — 3-column grid, hard edges ──
  renderTailwindFeaturedListings() {
    ...
    container.innerHTML = listings.map((l, idx) => {
      const imgUrl = Utils.placeholderImage(600, 400, l.rooms);
      const badge = l.type === 'satilik'
        ? '<span class="absolute left-0 top-0 bg-mint px-3 py-1.5 font-brand text-[10px] uppercase tracking-[0.023em] text-navy">SATILK</span>'
        ...
        <article class="listing group relative overflow-hidden bg-navy-deep reveal ${idx > 0 ? 'delay-1' : ''}">
          <a href="ilan-detay.html?id=${l.id}" class="block">
            ...
            <div class="p-6">
              ...
              <p class="mt-2 font-body text-[12px] text-white/40 flex items-center gap-1"><iconify-icon icon="lucide:map-pin"></iconify-icon> ${Utils.sanitize(l.location?.neighborhood || l.city || 'Nusaybin')}</p>
              <div class="mt-4 flex items-center justify-between border-t border-graphite/40 pt-4">
                <strong class="font-brand text-[18px] tracking-[0.023em] text-mint">₺${l.price.toLocaleString('tr-TR')}</strong>
                <span class="font-brand text-[12px] uppercase tracking-[0.023em] text-white/35">${l.rooms || '-'} · ${l.area} m²</span>
              </div>
            </div>
          </a>
        </article>
  ...
  // ── Listings Grid (ilanlar.html) — hard-edged cards on white surface ──
  renderTailwindListings() {
    ...
    if (searchInput) listings = listings.filter(l =>
      ...
      l.location.neighborhood.toLowerCase().includes(searchInput) ||
      l.features.some(f => f.toLowerCase().includes(searchInput))
    );
        ...
        <article class="listing reveal group overflow-hidden border border-ash/40 transition">
          <a href="ilan-detay.html?id=${l.id}" class="block">
            <div class="relative h-56 overflow-hidden">
              <img loading="lazy" src="${Utils.placeholderImage(400,300,l.rooms)}" alt="${Utils.sanitize(l.title)}" class="h-full w-full object-cover">
              ${badge}
            </div>
            <div class="p-6">
              ...
              <p class="mt-2 flex items-center gap-1 font-body text-[12px] text-steel">
                <iconify-icon icon="lucide:map-pin"></iconify-icon> ${Utils.sanitize(l.location.neighborhood)}, ${Utils.sanitize(l.location.city)}
              </p>
              <div class="mt-5 flex gap-4 border-t border-ash/30 pt-4 font-brand text-[12px] uppercase tracking-[0.023em] text-steel">
                ...
                <span>${l.area} m²</span>
                ${l.floor ? `<span>${Utils.sanitize(l.floor)}</span>` : ''}
              </div>
            </div>
          </a>
        </article>
  ...
  renderTailwindDetail() {
    ...
    const typeCat = `${listing.type === 'satilik' ? 'Satılık' : 'Kiralık'} · ${listing.category.charAt(0).toUpperCase() + listing.category.slice(1)}`;
    document.getElementById('detail-type-cat').textContent = typeCat;
    document.getElementById('detail-title').textContent = listing.title;
    document.getElementById('detail-location').textContent = `${listing.location.neighborhood}, ${listing.location.city}`;
    document.getElementById('detail-id').textContent = `İlan No: EE-${listing.id}`;
    document.getElementById('detail-desc').textContent = listing.description;

    document.getElementById('detail-aside-type').textContent = listing.type === 'satilik' ? 'Satılık' : 'Kiralık';
    document.getElementById('detail-price').textContent = `₺ ${listing.price.toLocaleString('tr-TR')}`;
    ...
    if(settings.phone) {
      ...
      document.getElementById('whatsapp-agent-link').href = `https://wa.me/${phoneClean}?text=${encodeURIComponent(listing.title + ' ilanı ile ilgili bilgi almak istiyorum.')}`;
      document.getElementById('phone-agent-link').href = `tel:${phoneClean}`;
      document.getElementById('aside-phone').textContent = settings.phone;
    }    // Image Gallery
    ...
    let galleryHTML = \
      <div class="group relative overflow-hidden lg:row-span-2">
        <img src="\" class="h-full min-h-[390px] w-full object-cover transition duration-700 group-hover:scale-[1.03]" onerror="this.src='\'">
      </div>
    \;
      ...
      topExtra.forEach(url => {
        galleryHTML += \
          <div class="group relative overflow-hidden hidden lg:block">
            <img src="\" class="h-full w-full object-cover transition duration-700 group-hover:scale-[1.03]">
          </div>
        \;
      });
    }

    document.getElementById('detail-images-grid').innerHTML = galleryHTML;
    ...
    if (videoSection) {
      if (listing.videoUrl) {
        ...
        document.getElementById('detail-video-container').innerHTML = \
          <iframe class="w-full aspect-video rounded-xl" src="\" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
        \;
      } else {
    ...
    document.getElementById('detail-specs').innerHTML = `
      ...
      <div class="border border-ash/40 p-5"><iconify-icon icon="lucide:ruler" class="text-[24px] text-mint"></iconify-icon><p class="mt-4 font-brand text-[12px] uppercase tracking-[0.023em] text-steel">Brüt Alan</p><strong class="mt-1 block font-brand text-[18px] uppercase tracking-[0.023em] text-navy">${listing.area} m²</strong></div>
    ...
    if(listing.features && listing.features.length > 0) {
      document.getElementById('detail-features').innerHTML = listing.features.map(f =>
        `<div class="flex items-center gap-3 font-body text-[14px] text-anvil"><iconify-icon icon="lucide:check" class="text-[18px] text-mint"></iconify-icon> ${f}</div>`
      ).join('');
    } else {
      document.getElementById('detail-features-section').classList.add('hidden');
    }

    // Related
    const related = DataManager.getAllListings().filter(l => l.id != id && l.category === listing.category).slice(0, 4);
    document.getElementById('relatedListingsGrid').innerHTML = related.map(l => `
      <a href="ilan-detay.html?id=${l.id}" class="group overflow-hidden border border-ash/40 transition hover:border-mint">
        <div class="relative h-48 overflow-hidden">
          <img src="${Utils.placeholderImage(400,300,l.rooms)}" class="h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]">
        </div>
        <div class="p-5">
          <h3 class="font-brand text-[14px] uppercase tracking-[0.023em] text-navy">${Utils.sanitize(l.title)}</h3>
          <p class="mt-2 font-brand text-[12px] uppercase tracking-[0.023em] text-steel">${l.rooms || '-'} · ${l.area} m²</p>
